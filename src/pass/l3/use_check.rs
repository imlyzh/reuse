...
use crate::{
    ir::l3_ir::{Bind, BindPattern, Body, Compute, Function, If, Match},
    types::Owned,
};
...
impl Body {
    pub fn use_check(&self, used_record: &mut Used) -> Result<(), String> {
        match self {
            ...
            Body::Dup(_, e) => e.use_check(used_record),
            Body::Drop(_, e) => e.use_check(used_record),
            Body::DropReuse(_, _, e) => e.use_check(used_record),
        }
    }
}
